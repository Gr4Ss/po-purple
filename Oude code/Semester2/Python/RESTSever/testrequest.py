...
# Add new parcel
r = requests.put(root + "/parcels/add", json=json.dumps({"secretkey":"SecretKey","newParcels":[[153,3,4]]}))
assert r.text == "OK"
# Add invalid new parcel
r = requests.put(root + "/parcels/add",json=json.dumps({"secretkey":"SecretKey","newParcels":[153,3,4]}))
assert r.text == "SORRY"
# Add invalid new parcel
r = requests.put(root + "/parcels/add",json=json.dumps({"secretkey":"SecretKey","newParcels":"Pieter"}))
assert r.text == "SORRY"
r = requests.get(root + "/parcels")
print "PARCELS:", r.json()
...
assert check_position(r.json()["positions"],"Paars",1,3)
# Send correctly position
r = requests.put(root + "/positions/Paars/3/2",data={"key":"245432FDSFQSG"})
assert r.text == 'OK'
r = requests.get(root + "/positions")
assert check_position(r.json()["positions"],"Paars",3,2)
# Delete correctly
